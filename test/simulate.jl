...
@testset "simulate, 1 qubit" begin
    @testset "function schedule, default anneal time, analytical solution" begin
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_CIRCULAR, 100, 4)
    ...
    @testset "function schedule, fast anneal time, analytical solution" begin
        ρ = simulate_fixed_order(one_spin_model, 0.5, AS_CIRCULAR, 1000, 4)
        @test isapprox(one_spin_ρ(0.5), ρ)
    end

    @testset "function schedule, slow anneal time, analytical solution" begin
        ρ = simulate_fixed_order(one_spin_model, 2.0, AS_CIRCULAR, 1000, 4)
    ...
    @testset "fractional field value" begin
        ...
        ρ = simulate_fixed_order(Dict((1,) => 0.5), 1.0, AS_CIRCULAR, 100, 4)
    ...
    @testset "field value above 1.0" begin
        ...
        ρ = simulate_fixed_order(Dict((1,) => 1.5), 1.0, AS_CIRCULAR, 100, 4)
    ...
    @testset "function schedule, constant terms" begin
        ...
        ρ = simulate_flexible_order(one_spin_model, 1.0, AS_CIRCULAR, 100, 4, constant_field_x = [1], constant_field_z = [1])
    ...
    @testset "function schedule, different initial state" begin
        ...
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_CIRCULAR, 100, 4, initial_state = [0,1])
    ...
    @testset "function schedule (AS_LINEAR), default anneal time" begin
        ...
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_LINEAR, 100, 4)
    ...
    @testset "function schedule (AS_QUADRATIC), default anneal time" begin
        ...
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_QUADRATIC, 100, 4)
    ...
    @testset "function schedule (AS_DW_QUADRATIC), default anneal time" begin
        ...
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_DW_QUADRATIC, 100, 4)
    ...
    @testset "function schedule, too few steps, analytical solution" begin
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_CIRCULAR, 10, 4)
    ...
    @testset "csv schedule pwq, analytical solution" begin
        ρ = simulate_fixed_order(one_spin_model, 1.0, AS_CIRCULAR_pwq_csv_1000, 100, 4)
    ...
    @testset "collect probability trajectory, nonadaptive" begin
        ρ_list = []
        steps=100
        ρ = simulate_fixed_order(one_spin_model, 1, AS_CIRCULAR, steps, 4, state_steps=ρ_list)
...
@testset "simulate, 2 qubit" begin
    @testset "function schedule, default anneal time, analytical solution" begin
        ρ = simulate_fixed_order(two_spin_model, 1.0, AS_CIRCULAR, 100, 4)
        @test isapprox(two_spin_ρ(1.0), ρ)
    end

    @testset "function schedule, fast anneal time, analytical solution" begin
        ρ = simulate_fixed_order(two_spin_model, 0.5, AS_CIRCULAR, 100, 4)
        @test isapprox(two_spin_ρ(0.5), ρ)
    end

    @testset "function schedule, slow anneal time, analytical solution" begin
        ρ = simulate_fixed_order(two_spin_model, 2.0, AS_CIRCULAR, 100, 4)
...
@testset "simulate, multi-qubit" begin
    ...
    @testset "2 qubit, function schedules (AS_CIRCULAR, AS_LINEAR, AS_QUADRATIC), near adiabatic limit" begin
        ...
        ρ = simulate_fixed_order(ising_model, annealing_time, AS_CIRCULAR, steps, 4)
        @test real(ρ[4,4]) >= 0.9999

        @test isapprox(min_ρ, ρ, atol=1e-2)
        @test !isapprox(min_ρ, ρ, atol=1e-3)

        ρ = simulate_fixed_order(ising_model, annealing_time, AS_LINEAR, steps, 4)
        @test real(ρ[4,4]) >= 0.9999

        @test isapprox(min_ρ, ρ, atol=1e-2)
        @test !isapprox(min_ρ, ρ, atol=1e-3)

        ρ = simulate_fixed_order(ising_model, annealing_time, AS_QUADRATIC, steps, 4)
        @test real(ρ[4,4]) >= 0.9999
    ...
    @testset "2 qubit, function schedules (AS_DW_QUADRATIC), near adiabatic limit" begin
        ...
        ρ = simulate_fixed_order(ising_model, annealing_time, AS_DW_QUADRATIC, steps, 4)
        @test real(ρ[4,4]) >= 0.9999
    ...
    @testset "2 qubit, csv schedule, near adiabatic limit" begin
        ...
        ρ = simulate_fixed_order(ising_model, 100.0, AS_CIRCULAR_pwl_csv_1000, 100, 4)
    ...
    @testset "2 qubit, function schedule, fractional values, near adiabatic limit" begin
        ...
        ρ = simulate_fixed_order(ising_model, 50.0, AS_CIRCULAR, 100, 4)
    ...
    @testset "3 qubit, csv schedule, near adiabatic limit" begin
        ...
        ρ_target = simulate_fixed_order(ising_model, annealing_time, AS_CIRCULAR, steps, 4)
        ρ = simulate_fixed_order(ising_model, annealing_time, AS_CIRCULAR_pwq_csv_1000, steps, 4)
...
@testset "simulate_de" begin
    ...
    @testset "2 qubit, function schedule" begin
        ...
        ρ = simulate_fixed_order(ising_model, 1.0, AS_CIRCULAR, 100, 4)
    ...
    @testset "2 qubit, function schedule, long annealing time" begin
        ising_model = Dict((1,) => -0.1, (2,) => -1, (1,2) => -1)
        ρ = simulate_fixed_order(ising_model, 1000.0, AS_CIRCULAR, 4000, 4)
    ...
    @testset "2 qubit, function schedule, adaptive tolerance" begin
        ising_model = Dict((1,) => -0.1, (2,) => -1, (1,2) => -1)
        ρ = simulate_fixed_order(ising_model, 100.0, AS_CIRCULAR, 500, 4)
